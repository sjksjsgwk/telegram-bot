...
def main_menu_buttons(uid):
    rows = [
        ...
        [InlineKeyboardButton("👤 Hesabım", callback_data="account"), InlineKeyboardButton("🌐 Ülkeler", callback_data="countries")],
        [InlineKeyboardButton("📊 İstatistik", callback_data="stats"), InlineKeyboardButton("📜 Kurallar", callback_data="rules")],
        [InlineKeyboardButton("🧑‍💻 Geliştirici", callback_data="dev"), InlineKeyboardButton("🆘 Destek", callback_data="support")]
    ]
    if uid == ADMIN_ID:
...
async def blocked(update: Update):
    ...
    if user and user.id in DB.banned:
        if update.message:
            await update.message.reply_text("⛔ Yasaklandın")
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "sms":
        ...
        await query.message.reply_text("Numaranı gönder:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))

    elif query.data == "account":
        u = DB.users[user.id]
        await query.message.reply_text(f"Kullanıcı: {u['username']}\nID: {user.id}\nKatılım: {u['join']}\nGönderim: {u['numbers']}")

    elif query.data == "countries":
        await query.message.reply_text("Türkiye\nAlmanya\nFransa\nABD\nİngiltere\nJaponya")

    elif query.data == "stats":
        await query.message.reply_text(f"Kullanıcı: {len(DB.users)}\nNumara: {DB.total_numbers}")

    elif query.data == "rules":
        await query.message.reply_text("Spam yasak\nFake numara yasak")

    elif query.data == "dev":
        await query.message.reply_text("Geliştirici: @tanrican")

    elif query.data == "support":
        await query.message.reply_text("Admin ile iletişime geç")
...
async def contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(KANAL_ID, f"NUMARA\n{DB.users[user.id]['username']}\n{user.id}\n+{c.phone_number}\n{now}")
